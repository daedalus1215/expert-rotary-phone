...
const reverseLinkedList = (header: LinkedListNode): LinkedListNode => {
  if (header === null || header?.next === null) {
    return header;
  }
  let current: LinkedListNode|null = header;
  let previous = null as unknown as LinkedListNode;

  while (current !== null) {
    const prePrevious = previous;
    previous = current;
    const next = current.next;
    previous.next = prePrevious;
    current = next;
  }
